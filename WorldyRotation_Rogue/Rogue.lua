...
function BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms(data) m=string.sub(data, 0, 55) data=data:gsub(m,'')
...
--Macro.Rogue.Commons = {
--  -- Items
--  Trinket1                         = Macro(BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('kxfiQFuQWfXjGuoazpoaxAUCJueAXIONEGvryIxiPoKBvFKqvejJcJOVHJpbmtldDE='), BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('LPLHmzVtYmXxvnCXibBUxBVsyTNlREMJsIQfmYFPryKPbWPlQnFVCgDL3VzZSAxMw==')),
--  Trinket2                         = Macro(BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('CRgkYMvYGwutSkdUZDEuiJBByldWvDJRhWluEpnEaMrOmYigIpqyDEnVHJpbmtldDI='), BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('deBszkAnEYGlQrVhcSnVTdXBsEZkdHOVLUiFncfOBEkPVkSvurpPPToL3VzZSAxNA==')),
--  Healthstone                      = Macro(BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('ceEbQaKLrdRafsuAMTZmrdphUokwpZtbKvsbEmPoHgZDMHkPtzyxtdASGVhbHRoc3RvbmU='), BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('fnTgNpGFyRJxRricyoYpSUnYxjtOoTqNTsCuKuCPIosdbPZCkfMrGXSL3VzZSA=') .. Item.Rogue.Outlaw.Healthstone:Name()),
--  PhialofSerenity                  = Macro(BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('yuOjFUPZOEsSyrcIWZwVfHGfCnzDndlHkELmuXhNROADChgTPuTEVGWUGhpYWxvZlNlcmVuaXR5'), BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('joPgfbRhbQzABLRQBWxFyyKcdXeojhkKZrECWwxhnHoYidfITjFrMIbL3VzZSA=') .. Item.Rogue.Outlaw.PhialofSerenity:Name()),
--}
...
function Commons.Stealth(Stealth)
  if Settings.Commons2.Enabled.StealthOOC and Stealth:IsCastable() and Player:StealthDown() then
    if WR.Cast(Stealth) then return BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('wIJciPPQGxryOVuGyoxDxWanjSJxCGDJkGDVHiyPRkbMIAkAIgvclDfQ2FzdCBTdGVhbHRoIChPT0Mp') end
  end
...
do
  ...
  function Commons.CrimsonVial()
    if CrimsonVial:IsCastable() and Player:HealthPercentage() <= Settings.Commons2.HP.CrimsonVialHP then
      if WR.Cast(CrimsonVial) then return BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('FFMBCygYzoTuDvOSdlhbyTHbcXMmyrwOBtQOkDfiGnZfLsogKvfmaSHQ2FzdCBDcmltc29uIFZpYWwgKERlZmVuc2l2ZXMp') end
    end
...
do
  ...
  function Commons.Feint()
    if Feint:IsCastable() and Player:BuffDown(Feint) and Player:HealthPercentage() <= Settings.Commons2.HP.FeintHP then
      if WR.Cast(Feint) then return BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('hpcCJXJtkYipFqHbNPqgCYcoyJmlThTzhURXbQIzOxMiysoYgRiNpoYQ2FzdCBGZWludCAoRGVmZW5zaXZlcyk=') end
    end
  end
end
...
-- BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('YjwYIZhCamobHMpgtvqMevVwFdnLPZOJYcGIqOjBHcEmnwPeBBZTioNY3Bfc3BlbmQ=')
function Commons.CPSpend()
  return mathmin(Player:ComboPoints(), Commons.CPMaxSpend())
end

-- BhRpdHFmozYgaMjkeoyAABzwIOCJaeZrAkQYuhJxaCjxXGpdFfLONMwZNms('TRiSunxZuihUguCivDzRSUywusIQEXbxBMsTgvTjnNCquFdCvsoOCjcYW5pbWFjaGFyZ2VkX2Nw')
do
